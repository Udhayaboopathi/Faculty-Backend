...
export async function createAward(req, res) {
  try {
    ...
    res.json({
      success: true,
      award: { Title, A_date, Level, Sponcer, Spon_Address },
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
export async function updateAward(req, res) {
  try {
    ...
    if (parseInt(existingAward[0].emp_id) !== parseInt(current_user.EMP_ID)) {
      return res.status(403).json({ error: "You are not authorized to update this award" });
    }
...
export async function deleteAward(req, res) {
  try {
    ...
    if (parseInt(existingAward[0].emp_id) !== parseInt(current_user.EMP_ID)) {
      return res.status(403).json({ error: "You are not authorized to delete this award" });
    }
