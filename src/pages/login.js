...
export function registerLoginRoute(app) {
  ...
  app.post("/login/teaching", async (req, res) => {
    ...
    if (!off_email || !password) {
      return res
        .status(400)
        .json({ error: "off_email and password are required" });
    }
    try {
      const rows = await db
        .select()
      ...
      const user_role_query = await db
        .select()
        .from(empRoleMaster)
        .where(eq(empRoleMaster.emp_id, empId))
        .limit(1);
      const user_role_id = user_role_query[0]?.role_id ?? 1;

      const user_role_name_query = await db
        .select()
        .from(roleMaster)
        .where(eq(roleMaster.id, user_role_id))
        .limit(1);
      const user_role_name = user_role_name_query[0]?.role ?? "teaching";

      const da_percentage_query = await db
        .select()
        .from(da_percentage)
        .orderBy(eq(da_percentage.id, 1))
        .limit(1);
      const da_percent = da_percentage_query[0]?.da_percent || 0;

      const employee_dept_id_query = await db
        .select()
        .from(empDeptMaster)
        .where(eq(empDeptMaster.emp_id, empId))
        .limit(1);

      const employee_dept_id = employee_dept_id_query[0]?.dept_id || null;

      const dept_name_query = await db
        .select()
        .from(departmentMaster)
        .where(eq(departmentMaster.id, employee_dept_id))
        .limit(1);

      const dept_name = dept_name_query[0]?.department || "";
  ...
  app.post("/login/nonteaching", async (req, res) => {
    try {
      ...
      const da_percent_query = await db
        .select()
        .from(da_percentage)
        .orderBy(eq(da_percentage.id, 1))
        .limit(1);
